...
#3D plot
library(plot3D)
points3D(pca_x$x[,1], pca_x$x[,2],  pca_x$x[,3],col = y, pch = ".")
```
#clustering
```{r}
...
c1 <- which(chat == 1)
c1.x <- all.x[c1, ]
first4wk.c1 <- first4wk[c1, ]
#cluster 2
c2 <- which(chat == 2)
c2.x <- all.x[c2, ]
first4wk.c2 <- first4wk[c2, ]
```
#PCA on cluster1
pca.c1 <- prcomp(c1.x)
plot(pca.c1$x[, 1:2], col = c1.y, main = "Cluster1")
legend("topleft", c("negative", "positive"), col = c(1,2), lty = 1)
#3D
points3D(pca.c1$x[,1], pca.c1$x[,2],  pca.c1$x[,3],col = c1.y)
```
#PCA on cluster2
```{r}
c2 <- which(chat==2)
...
table(c2.y)
pca.c2 <- prcomp(c2.x)
plot(pca.c2$x[, 1:2], col = c2.y, main = "Cluster2 Outcome")
legend("topleft", c("negative", "positive"), col = c(1,2), lty = 1)
plot(pca.c2$x[, 2:3], col = c2.y, main = "Cluster2 Outcome")
legend("topleft", c("negative", "positive"), col = c(1,2), lty = 1)
plot(pca.c2$x[, c(1,3)], col = c2.y, main = "Cluster2 Outcome")
legend("topleft", c("negative", "positive"), col = c(1,2), lty = 1)
#3D
points3D(pca.c2$x[,1], pca.c2$x[,2],  pca.c2$x[,3],col = c2.y)
```
#A clustering on cluster2
```{r}
cluster2.0 <- kmeans(c2.x, 2)
chat2.0 <- cluster2.0$cluster
yhat2.0 <- factor(chat2.0, levels = unique(chat2.0), labels = levels(c2.y))
par(mfrow=c(1,2))
plot(pca.c2$x[, 1:2], col = c2.y, main = "True Outcome")
legend("topleft", c("negative", "positive"), col = c(1,2), lty = 1)
plot(pca.c2$x[, 1:2], col = yhat2.0, main = "Clustering Outcome")
legend("topleft", c("cluster1", "cluster2"), col = c(1,2), lty = 1)
table(c2.y[chat2.0==1])
table(c2.y[chat2.0==2])
```
#Build a model on the second cluster
#split data
```{r}
ind <- sample.int(nrow(c2.x), size = 0.7*nrow(c2.x))
train.x <- c2.x[ind, ]
train.y <- c2.y[ind]
test.x <- c2.x[-ind, ]
test.y <- c2.y[-ind]
...
dftrain <- model.matrix(f, dftrain)
```
#packages
```{r}
library(glmnet)
...
```{r}
lassolg <- cv.glmnet(dftrain, train.y, 
                       family = "binomial",
                       type.measure = "class",
                       nfolds = 10,
                       alpha = 1)
plot(lassolg$lambda, lassolg$cvm, type = "l")
min(lassolg$cvm)
```
